...
def sell_crypt(bot, client):
    ...
    bot.send_message(client, f'Заявки на продажу принимаются в ручном режиме @{settings.support_contact}', reply_markup=None)
...
def choice_payment_method(bot, client, message):
    if message.text == '❌ Отмена':
        # Stats.
        CanceledOrder.objects.create(client=client, pay_value=client.meta['order_sell']['pay_value_with_percent'])
...
def accept_sell(bot, client, message):
    if message.text == '❌ Отмена':
        ...
        bot.send_message(client, '⏳ Ожидайте, идет подбор реквизитов', reply_markup=telebot.types.ReplyKeyboardRemove())
