...
class Wallet:
    ...
    @classmethod
    def get_course(cls):
        course = cache.get(f'{cls.currency}-{cls.network}')

        if not course:
            url = cls.base_url + '/rates'
            params = {'coin': f'{cls.currency}-{cls.network}'}
            ...
            try:
                ...
                course = usdt_course
                cache.set(f'{cls.currency}-{cls.network}', course, 120)
                print('rapira')
    ...
    def get_address(cls, wallets=False):
        ...
        data = {'coin': f'{cls.currency}-{cls.network}'}
    ...
    def send_crypt(cls, address, value, fee, wallets=False):
        ...
        cache_key = f"{address}:{value}:{f'{cls.currency}-{cls.network}'}"

        if cache.get(cache_key):
            return '⚠️ Повторная отправка'

        url = cls.base_url + '/withdrawal'
        data = {'coin': f'{cls.currency}-{cls.network}', 'amount': value, 'address': address}
    ...
    def get_history(cls, wallets=False):
        ...
        params = {'coin': f'{cls.currency}-{cls.network}', 'limit': 50}
        response = requests.get(url, headers=headers, params=params)
    ...
    def get_receive_history(cls, wallets=False):
        ...
        url = cls.base_url + '/history'
        params = {'coin': f'{cls.currency}-{cls.network}', 'category': 'receive', 'limit': 100}
        response = requests.get(url, headers=headers, params=params)
    ...
    def get_min_deposit(cls, wallets=False):
        ...
        for item in data:
            if item['name'] == f'{cls.currency}-{cls.network}':
