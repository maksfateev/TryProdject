...
class Wallet:
	...
	def send_crypt(address, value, fee):
		try:
			transaction = client.create_withdrawal(Wallet.currency, str(value), address, priority='high')
		except InsufficientFundsException:
			return 'Недостаточно BTC для отправки'
		except BadAddressException:
			return 'Некорректный BTC адрес'
		...
		result = {
			...
			'tx_link': 'https://mempool.space/ru/address/' + address,
	...
	@staticmethod
	def get_history():
		data = {'currency': Wallet.currency, 'limit': 50}
