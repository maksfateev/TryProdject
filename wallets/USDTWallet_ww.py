...
class Wallet:
	...
	def get_balance():
		try:
			balance = client.wallet_balance(Wallet.currency + Wallet.network)
	...
	def get_address():
		try:
			address = client.generate_address(Wallet.currency + Wallet.network, 'https://artbossbot.ru/callback')
	...
	def send_crypt(address, value, fee):
		try:
			transaction = client.create_withdrawal(Wallet.currency + Wallet.network, str(value), address)
		...
		result = {
			...
			'tx_link': 'https://tronscan.org/#/address/' + address,
	...
	def get_history():
		try:
			...
			outcomig_txs = []
			for tr in history:
				if tr['currency'] == Wallet.currency or tr['currency'] == Wallet.currency + Wallet.network:
					history_item = {
